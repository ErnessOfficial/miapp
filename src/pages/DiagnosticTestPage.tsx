...
const DiagnosticTestPage: React.FC = () => {
  ...
  const [answers, setAnswers] = useState<AnswerValue[]>(Array(diagnosticQuestions.length).fill(null));
  ...
  return (
    <div className="max-w-3xl mx-auto p-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <button type="submit" className="mt-6 px-4 py-2 bg-animik-blue text-white rounded">
          {t('diagnostic.submit')}
        </button>
      </form>
      {submitted && <div className="mt-6 text-green-600">{t('diagnostic.submissionMessage')}</div>}
    </div>
  );
};
